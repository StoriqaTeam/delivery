...
impl CompaniesPackagesRaw {
    pub fn to_model(self) -> Result<CompanyPackage, FailureError> {
        ...
        match shipping_rate_source {
            ShippingRateSourceRaw::NotAvailable => {
                Ok(CompanyPackage {
                    id,
                    company_id,
                    package_id,
                ...
                let shipping_rates = shipping_rates
                    ...
                    .and_then(|rates|
                        serde_json::from_value::<Vec<ShippingRate>>(rates)
                            .map_err(|e| e.context(format!("Failed to parse shipping rates for CompanyPackage with id = {}", id)))
                    );

                let dimensional_factor = dimensional_factor
                    .ok_or(format_err!("Dimensional factor is missing for CompanyPackage with id = {}", id));

                shipping_rates.and_then(|rates|
                    dimensional_factor.map(|dimensional_factor|
                        CompanyPackage {
                            id,
                            company_id,
                            package_id,
                            shipping_rate_source: ShippingRateSource::Static(ShippingRates {
                                dimensional_factor,
                                rates,
                            })
                        })
            ...
            ShippingRateSourceRaw::OnDemand => {
                Err(format_err!("CompanyPackages with on-demand sources of shipping rates \
                                 are not yet supported (CompanyPackage id = {})", id))
            }
        }
    }
}
...
impl NewCompaniesPackagesRaw {
    pub fn from_model(new_company_package: NewCompanyPackage) -> Result<Self, FailureError> {
        ...
        match shipping_rate_source.unwrap_or_default() {
            ...
            ShippingRateSource::Static(ShippingRates { dimensional_factor, rates }) =>
